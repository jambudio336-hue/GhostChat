...
export default function CallsScreen() {
  ...
  const [quality, setQuality] = useState<NetworkQuality>({ label: "MENUNGGU", color: "#8f96a4", rttMs: null, packetLoss: null, route: "MENUNGGU" });

  useEffect(() => () => session?.close(), [session]);
  useEffect(() => {
    if (!session) { setQuality({ label: "MENUNGGU", color: "#8f96a4", rttMs: null, packetLoss: null, route: "MENUNGGU" }); return; }
    let active = true;
    const refresh = async () => { try { const next = await session.getQuality(); if (active) setQuality(next); } catch { /* stats unavailable until native peer is connected */ } };
    refresh();
  ...
  const startCall = async () => {
    try {
      ...
      setOutgoing(offerText);
      await Clipboard.setStringAsync(offerText);
      setStatus("Offer siap dan otomatis tersalin. Kirim ke partner.");
    } catch (error) {
  ...
  const acceptOffer = async () => {
    try {
      const raw = incoming.trim() || await Clipboard.getStringAsync();
      if (!raw.trim()) throw new Error("Tempel offer dari perangkat partner terlebih dahulu.");
      setIncoming(raw);
      ...
      setOutgoing(answerText);
      await Clipboard.setStringAsync(answerText);
      setStatus("Answer siap dan otomatis tersalin. Kirim kembali ke pembuat offer.");
    } catch (error) {
  ...
  const applyAnswer = async () => {
    try {
      const raw = incoming.trim() || await Clipboard.getStringAsync();
      if (!raw.trim()) throw new Error("Tempel answer dari perangkat partner terlebih dahulu.");
      setIncoming(raw);
      const payload = JSON.parse(raw) as SignalPayload;
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]}><ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.explainer}><MaterialIcons name="security" size={22} color="#ff4968" /><View style={{ flex: 1 }}><Text style={styles.explainerTitle}>P2P + TURN relay aktif</Text><Text style={styles.explainerText}>Offer/answer ditukar manual. GhostChat mencoba koneksi langsung, lalu memakai Open Relay TURN gratis (UDP/TCP/TLS) jika jaringan Surabaya–Jakarta tidak bisa menembus NAT.</Text></View></View>
    ...
    <View style={styles.qualityCard}><View style={styles.qualityHeading}><Text style={styles.fieldLabel}>KUALITAS JARINGAN · REAL-TIME</Text><View style={styles.qualityBadge}><View style={[styles.qualityDot, { backgroundColor: quality.color }]} /><Text style={[styles.qualityLabel, { color: quality.color }]}>{quality.label}</Text></View></View><View style={styles.qualityMetrics}><Text style={styles.metric}><Text style={styles.metricValue}>{quality.rttMs === null ? "—" : `${quality.rttMs} ms`}</Text>{"\n"}LATENCY</Text><Text style={styles.metric}><Text style={styles.metricValue}>{quality.packetLoss === null ? "—" : `${quality.packetLoss.toFixed(1)}%`}</Text>{"\n"}PACKET LOSS</Text><Text style={styles.metric}><Text style={styles.metricValue}>{quality.route}</Text>{"\n"}JALUR</Text></View></View>
    <Text style={styles.section}>SIGNALING MANUAL</Text>
    <View style={styles.card}><Text style={styles.fieldLabel}>PAYLOAD DARI PARTNER</Text><TextInput value={incoming} onChangeText={setIncoming} placeholder="Tempel JSON offer atau answer di sini" placeholderTextColor="#777e8a" multiline style={styles.textArea} /><Pressable style={({ pressed }) => [styles.pasteButton, pressed && styles.pressed]} onPress={async () => setIncoming(await Clipboard.getStringAsync())}><MaterialIcons name="content-paste" size={16} color="#ff4968" /><Text style={styles.secondaryText}>TEMPEL DARI CLIPBOARD</Text></Pressable><View style={styles.buttonRow}><Pressable style={({ pressed }) => [styles.secondaryButton, pressed && styles.pressed]} onPress={acceptOffer}><MaterialIcons name="call-received" size={17} color="#ff4968" /><Text style={styles.secondaryText}>TERIMA OFFER</Text></Pressable><Pressable style={({ pressed }) => [styles.secondaryButton, pressed && styles.pressed]} onPress={applyAnswer}><MaterialIcons name="link" size={17} color="#ff4968" /><Text style={styles.secondaryText}>TERAPKAN ANSWER</Text></Pressable></View></View>
