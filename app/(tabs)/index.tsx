...
import { Alert, AppState, FlatList, ImageBackground, Linking, Modal, Pressable, StyleSheet, Text, TextInput, View } from "react-native";
...
import { sendPeerFile, sendPeerMessage, sendPresence, sendReaction, sendTyping, subscribePeerMessages, subscribePeerPresence, subscribePeerRead, subscribePeerReaction, subscribePeerTyping, type ChatPacket } from "@/lib/chat-channel";

const WA_NUMBER = "6285262965282";
const INTRO_KEY = "ghostchat_intro_seen";

type Message = { id: string; text: string; mine?: boolean; time: string; kind?: ChatPacket["kind"]; uri?: string; name?: string; status?: "sent" | "read"; replyTo?: ChatPacket["replyTo"]; forwarded?: boolean; reactions?: string[] };
...
export default function HomeScreen() {
  ...
  const [lastSeen, setLastSeen] = useState("baru saja");
  ...
  useEffect(() => { const unsubscribe = subscribePeerMessages((packet: ChatPacket) => setMessages((current) => [...current, packet])); const unsubscribeTyping = subscribePeerTyping(setPartnerTyping); const unsubscribeRead = subscribePeerRead((id) => setMessages((current) => current.map((message) => message.id === id ? { ...message, status: "read" } : message))); const unsubscribeReaction = subscribePeerReaction((id, emoji) => setMessages((current) => current.map((message) => message.id === id ? { ...message, reactions: [...(message.reactions ?? []), emoji] } : message))); const unsubscribePresence = subscribePeerPresence((online, seen) => { setPartnerOnline(online); if (seen) setLastSeen(seen); }); return () => { unsubscribe(); unsubscribeTyping(); unsubscribeRead(); unsubscribeReaction(); unsubscribePresence(); }; }, []);
  useEffect(() => { const subscription = AppState.addEventListener("change", (state) => sendPresence(state === "active")); return () => subscription.remove(); }, []);
  useEffect(() => { requestRecordingPermissionsAsync().then((result) => { if (result.granted) setAudioModeAsync({ playsInSilentMode: true, allowsRecording: true }); }); }, []);
  ...
  const sendMessage = () => {
    ...
    const replyTo = replyingTo ? { id: replyingTo.id, text: replyingTo.text } : undefined;
    if (!sendPeerMessage(text, id, { replyTo, forwarded: Boolean(forwarding) })) {
      Alert.alert("Belum terhubung", "Hubungkan panggilan WebRTC terlebih dahulu agar pesan terkirim ke perangkat partner.");
      return;
    }
    setMessages((current) => [...current, { id, text, mine: true, time: "sekarang", status: "sent", replyTo, forwarded: Boolean(forwarding) }]);
    setDraft("");
    setReplyingTo(null);
    setForwarding(null);
    sendTyping(false);
  };
  const handleDraftChange = (text: string) => { setDraft(text); sendTyping(text.trim().length > 0); };
  const reactToMessage = (message: Message) => { const emoji = "❤️"; if (sendReaction(message.id, emoji)) setMessages((current) => current.map((item) => item.id === message.id ? { ...item, reactions: [...(item.reactions ?? []), emoji] } : item)); };
  const forwardMessage = (message: Message) => { setForwarding(message); setDraft(message.text); Alert.alert("Teruskan pesan", "Pesan siap diteruskan. Tekan tombol kirim untuk mengirimkannya ke partner."); };
  const copyMessage = async (text: string) => {
    await Clipboard.setStringAsync(text);
    Alert.alert("Pesan disalin", "Teks pesan sudah masuk ke clipboard.");
  };

  const addAttachment = async (asset: { uri: string; name?: string; mimeType?: string; kind?: ChatPacket["kind"] }) => {
    const sent = await sendPeerFile(asset);
    if (!sent) { Alert.alert("Belum terhubung", "Hubungkan WebRTC terlebih dahulu sebelum mengirim file."); return; }
  ...
  return (
    <ScreenContainer edges={["top", "left", "right", "bottom"]} containerClassName="bg-background">
      <View style={styles.screen}>
        ...
        <View style={styles.chatHeader}><View><Text style={styles.sectionLabel}>CONVERSATION</Text><Text style={styles.chatTitle}>Night Owl <Text style={styles.lock}>⌁</Text></Text><View style={styles.presenceRow}><View style={[styles.presenceDot, partnerOnline ? styles.presenceOnline : styles.presenceOffline]} /><Text style={styles.presenceText}>{partnerOnline ? "Online" : `Offline · terakhir aktif ${lastSeen}`}</Text></View></View><View style={styles.deviceTag}><MaterialIcons name="smartphone" size={14} color="#9298a6" /><Text style={styles.deviceText}>ANDROID · 5G</Text></View></View>
        ...
        <ImageBackground source={chatBackground} style={styles.chatBackground} imageStyle={styles.chatBackgroundImage}>
          ...
          {partnerTyping ? <View style={styles.typingPill}><View style={styles.typingDot} /><Text style={styles.typingText}>Partner sedang mengetik…</Text></View> : null}
          <FlatList
            data={messages}
            ...
            renderItem={({ item }) => (
              <View style={[styles.messageRow, item.mine && styles.messageRowMine]}>
                ...
                <Pressable onLongPress={() => copyMessage(item.text)} delayLongPress={420} onPress={() => reactToMessage(item)} style={[styles.bubble, item.mine ? styles.mineBubble : styles.theirBubble]}>{item.replyTo ? <View style={styles.replyPreview}><Text style={styles.replyLabel}>Membalas</Text><Text style={styles.replyText} numberOfLines={1}>{item.replyTo.text}</Text></View> : null}{item.forwarded ? <Text style={styles.forwardedLabel}>↗ Diteruskan</Text> : null}<Text style={styles.messageText}>{item.kind === "photo" ? "📷 FOTO" : item.kind === "voice" ? "🎙 PESAN SUARA" : item.text}</Text><View style={styles.messageMeta}><Text style={styles.messageTime}>{item.time} {item.mine ? (item.status === "read" ? "✓✓" : "✓") : ""}</Text>{item.reactions?.length ? <Text style={styles.reactionBadge}>{item.reactions.join(" ")}</Text> : null}</View></Pressable>
                <View style={styles.messageActions}><Pressable onPress={() => setReplyingTo(item)}><Text style={styles.actionChip}>BALAS</Text></Pressable><Pressable onPress={() => forwardMessage(item)}><Text style={styles.actionChip}>TERUSKAN</Text></Pressable></View>
              </View>
            )}
          />
        </ImageBackground>

        {replyingTo ? <View style={styles.composerContext}><Text style={styles.contextLabel}>MEMBALAS</Text><Text style={styles.contextText} numberOfLines={1}>{replyingTo.text}</Text><Pressable onPress={() => setReplyingTo(null)}><MaterialIcons name="close" size={18} color="#aeb4c0" /></Pressable></View> : null}
        {forwarding ? <View style={styles.composerContext}><Text style={styles.contextLabel}>MENERUSKAN</Text><Text style={styles.contextText} numberOfLines={1}>{forwarding.text}</Text><Pressable onPress={() => setForwarding(null)}><MaterialIcons name="close" size={18} color="#aeb4c0" /></Pressable></View> : null}
        <View style={styles.composer}>
          <Pressable style={styles.attach} onPress={() => setShowAttachmentMenu((value) => !value)}><MaterialIcons name="add" size={23} color="#ff3355" /></Pressable>
          <TextInput value={draft} onChangeText={handleDraftChange} placeholder="Tulis pesan terenkripsi..." placeholderTextColor="#777d8a" style={styles.input} onSubmitEditing={sendMessage} returnKeyType="send" />
          <Pressable style={styles.voice} onPress={toggleVoice}><MaterialIcons name={recorderState.isRecording ? "stop" : "mic"} size={20} color={recorderState.isRecording ? "#ff4968" : "#c3c7d0"} /></Pressable>
          <Pressable style={styles.send} onPress={sendMessage}><MaterialIcons name="arrow-upward" size={21} color="#fff" /></Pressable>
        </View>

        {showAttachmentMenu ? <View style={styles.attachmentMenu}><Pressable onPress={() => pickPhoto(false)} style={styles.attachmentAction}><MaterialIcons name="photo-library" size={18} color="#ff4968" /><Text style={styles.actionText}>GALERI</Text></Pressable><Pressable onPress={() => pickPhoto(true)} style={styles.attachmentAction}><MaterialIcons name="photo-camera" size={18} color="#ff4968" /><Text style={styles.actionText}>KAMERA</Text></Pressable><Pressable onPress={pickDocument} style={styles.attachmentAction}><MaterialIcons name="insert-drive-file" size={18} color="#ff4968" /><Text style={styles.actionText}>DOKUMEN</Text></Pressable></View> : null}
