...
import { sendPeerFile, sendPeerMessage, subscribePeerMessages, type ChatPacket } from "@/lib/chat-channel";

const WA_NUMBER = "6285262965282";
const INTRO_KEY = "ghostchat_intro_seen";

type Message = { id: string; text: string; mine?: boolean; time: string; kind?: ChatPacket["kind"]; uri?: string; name?: string };
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={[styles.messageRow, item.mine && styles.messageRowMine]}>
                ...
                <Pressable onLongPress={() => copyMessage(item.text)} delayLongPress={420} style={[styles.bubble, item.mine ? styles.mineBubble : styles.theirBubble]}><Text style={styles.messageText}>{item.kind === "photo" ? "📷 FOTO" : item.kind === "voice" ? "🎙 PESAN SUARA" : item.text}</Text><Text style={styles.messageTime}>{item.time} {item.mine ? "✓✓" : ""}</Text></Pressable>
              </View>
            )}
          />
        </ImageBackground>
